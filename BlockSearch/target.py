...
class Target:
    ...
    def get_mesh(self):
        mesh = deepcopy(TARGET_MESH)
        orient_mesh(mesh, rotation=(0, 0, 0), translation=self.position)
        return mesh
    ...
    def __str__(self):
        return "{};{}".format(np.array(self.position, dtype=int),
                              "REACHED" if self.is_reached else "NOT_REACHED")